...
/*?>$user_right=mysqli_fetch_array(mysqli_query($link,"SELECT  `views` FROM `user_rights` WHERE `uid`='".mysqli_real_escape_string($link,$_SESSION['login']['id'])."' and `pagename`='wp_start'"));
<?php *//*?>mysqli_query($link,"INSERT INTO `history_log`(`h_id`, `time`, `operation`,`tab_name`,`url`) VALUES ('$_SESSION[h_id]','$time_stamp','Whatsapp Server Start','SMS','sms/wpserver')");<?php */ //}	
...
if($is_conn=="Y")
{
    $result=mysqli_query($link,"SELECT * FROM `sms` WHERE `sms_status`='pending' LIMIT 5");
    if(mysqli_affected_rows($link)>0)
    {
        while($row=mysqli_fetch_array($result)) {
            ...
            {
                mysqli_query($link,"UPDATE `sms` SET `wp_status`='done',`wp_senttime`=NOW(),`sms_status`='done' WHERE `id`='$row[id]'");
            }
            else
            {
