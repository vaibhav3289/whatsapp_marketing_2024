...
function del_sms(val)
{
	document.getElementById('gefinal').value=val;
       ...
       {
		   document.getElementById("smsid").action='<?php echo $path;?>phpfiles/del_send_data.php';
   ...
   <strong>Group Message Report</strong>
     ...
     <div class="col-sm-6 col-xs-6 float-right">
        <ol class="breadcrumb float-right">
            <li class="breadcrumb-item"><a class="btn btn-info btn-sm" href="<?php echo $path;?>add_new_msg.php" ><i class="fa fa-plus"></i>&nbsp;&nbsp;Group Message </a></li>&nbsp;&nbsp;
            <a class="btn btn-primary btn-sm" href="<?php echo $path;?>one_mg_report.php" >Single Message  </a>
        </ol>
      </div> </div>   
</section>
      ...
      <table class="table table-sm table-striped table-bordered table-hover" id="example1">
        ...
        <tbody>
            ...
            $i=1;	
                ...
                $sql="select * from `sms` where date_format(`hdate`,'%Y-%m-%d')>='$gtdate' and date_format(`hdate`,'%Y-%m-%d')<='$gfdate'  ";
          
            $res=mysqli_query($link,$sql);
            while($row=mysqli_fetch_array($res)) {

                list($group)=mysqli_fetch_array(mysqli_query($link,"SELECT `g_name` FROM `groupwm` WHERE `g_id`='$row[g_id]' "));
